...
class Plot_Reproduce_Performance():
    def __init__(self, DIR, n_img_x=8, img_w=28, resize_factor=1.0):
        self.DIR = DIR

        assert n_img_x > 0

        self.n_img_x = n_img_x
        self.n_tot_imgs = n_img_x
    ...
    def save_samples(self, samples, name='result.csv'):
        np.savetxt(self.DIR + "/"+name, samples,fmt='%.4f')

    def save_mse(self, samples, fobj):
        np.savetxt(fobj, samples,fmt='%.4f')


class Plot_Manifold_Learning_Result():
    def __init__(self, DIR, n_img_x=20, img_w=28, resize_factor=1.0, z_range=4):
        self.DIR = DIR
