...
def parse_args():
    ...
    parser.add_argument('--results_path', type=str, default='/scratch/gobi1/shalmali/defaultCredit/reg_vae',
                        help='File path of output images')

    parser.add_argument('--add_noise', type=bool, default=False, help='Boolean for adding salt & pepper noise to input image')

    parser.add_argument('--dim_z', type=int, default='10', help='Dimension of latent vector', required = True)

    parser.add_argument('--n_hidden', type=int, default=100, help='Number of hidden units in MLP')
    ...
    parser.add_argument('--PMLR_z_range', type=float, default=1.0,
...
def main(args):
    ...
    if PRR:
        PRR = plot_utils.Plot_Reproduce_Performance(RESULTS_DIR, PRR_n_img_x, IMAGE_SIZE_MNIST, PRR_resize_factor)
    ...
    if PMLR and dim_z == 2:
        ...
        PMLR = plot_utils.Plot_Manifold_Learning_Result(RESULTS_DIR, PMLR_n_img_x, IMAGE_SIZE_MNIST, PMLR_resize_factor, PMLR_z_range)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            print("epoch %d: L_tot %03.2f Neg L_likelihood %03.2f KL_divergence %03.2f" % (epoch, tot_loss, loss_likelihood, loss_divergence))
            ...
            if min_tot_loss > tot_loss or epoch+1 == n_epochs:
                ...
                if PRR:
                    ...
                    PRR.save_samples(y_PRR, name="/PRR_epoch_%02d" %(epoch) + ".csv")
                    PRR.save_mse(np.reshape(np.asarray([np.mean(np.linalg.norm(x_PRR-y_PRR,2,1)**2,axis=0),loss_likelihood]),[1,-1]), fname)
                ...
                if PMLR and dim_z == 2:
                    ...
                    PMLR.save_samples(y_PMLR, name="/PMLR_epoch_%02d" % (epoch) + ".csv")
